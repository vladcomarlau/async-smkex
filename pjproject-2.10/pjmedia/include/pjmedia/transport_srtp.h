...
/**
 * This method sets the SRTP out-of-band key setup callback
 */
typedef int (*crypto_oob_key_and_id_t)(char *key, int key_len, char *id, int max_id_len);
typedef int (*crypto_oob_key_from_id_t)(const char *id, int id_len, char *key, int key_len);
...
/**
 * Set SRTP out-of-band key and id callback. The callback obtains the
 * out-of-band crypto key and an associated key ID.
 * 
 * @param cipher_name Cipher name for which callback will be installed
 * @param oob_key_and_id Pointer to callback method
 * 
 */

PJ_DECL(pj_status_t) pjmedia_srtp_set_oob_key_and_id(pj_str_t cipher_name,
                            crypto_oob_key_and_id_t oob_key_and_id);


/**
 * Set SRTP callback which obtains out-of-band key from an ID
 * 
 * @param cipher_name Cipher name for which callback will be installed
 * @param oob_key_from_id Pointer to callback method
 * 
 */
PJ_DECL(pj_status_t) pjmedia_srtp_set_oob_key_from_id(pj_str_t cipher_name,
