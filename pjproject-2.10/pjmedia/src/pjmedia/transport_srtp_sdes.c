					      ...
					      int tag)
{
    ...
    char oob_key_id[MAX_KEY_LEN];
	int print_len;
	pj_bool_t is_oob_key = PJ_FALSE;
	...
	*buffer_len = 0;
	...
	/* If OOB key is enabled, obtain the key from the application layer */
	if (crypto_suites[cs_idx].oob_key_and_id) {
		status = crypto_suites[cs_idx].oob_key_and_id(key,
										crypto_suites[cs_idx].cipher_key_len,
										oob_key_id, MAX_KEY_LEN);
		if (status != PJ_SUCCESS) {
			PJ_PERROR(4,(THIS_FILE, status,
						"Failed to obtain OOB crypto key"));
			return PJMEDIA_SRTP_ENOTSUPCRYPTO;
		}
    ...
    /* Print the crypto attribute value (OOB key is already is ASCII key identifier). */
    print_len = pj_ansi_snprintf(buffer, *buffer_len, "%d %s %s:%s",
				   tag,
				   crypto_suites[cs_idx].name,
				   is_oob_key == PJ_TRUE ? "oob" : "inline",
				   is_oob_key == PJ_TRUE ? oob_key_id : b64_key);
				     ...
				     int *tag)
{
    ...
    *tag = pj_strtoul(&token);
	...
	if (is_oob_key == PJ_FALSE) {
		...
		if (!crypto_suites[cs_idx].oob_key_from_id) {
			PJ_PERROR(4,(THIS_FILE, status,
				"Missing Out-of-band key callback"));
			return PJMEDIA_SRTP_ENOTSUPCRYPTO;
		}
		
		status = crypto_suites[cs_idx].oob_key_from_id(token.ptr, token.slen,
											key, crypto_suites[cs_idx].cipher_key_len);
		if (status != PJ_SUCCESS) {
			PJ_PERROR(4,(THIS_FILE, status,
				"Cannot obtain Out-of-band key"));
			return PJMEDIA_SRTP_ENOTSUPCRYPTO;
		}
