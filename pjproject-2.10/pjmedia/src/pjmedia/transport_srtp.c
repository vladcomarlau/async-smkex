...
typedef struct crypto_suite
{
    ...
    crypto_oob_key_and_id_t oob_key_and_id;
    crypto_oob_key_from_id_t oob_key_from_id;
} crypto_suite;
...
PJ_DEF(pj_status_t) pjmedia_srtp_set_oob_key_and_id(pj_str_t cipher_name,
                            crypto_oob_key_and_id_t oob_key_and_id)
{
    int cs_idx = get_crypto_idx(&cipher_name);

    /* check crypto name */
    if (cs_idx == -1)
        return PJMEDIA_SRTP_ENOTSUPCRYPTO;

    crypto_suites[cs_idx].oob_key_and_id = oob_key_and_id;

    return PJ_SUCCESS;
}

PJ_DEF(pj_status_t) pjmedia_srtp_set_oob_key_from_id(pj_str_t cipher_name,
                            crypto_oob_key_from_id_t oob_key_from_id)
{
    int cs_idx = get_crypto_idx(&cipher_name);
    ...
    crypto_suites[cs_idx].oob_key_from_id = oob_key_from_id;
